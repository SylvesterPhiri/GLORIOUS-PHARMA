...
interface Product {
  ...
  price: number;
  expiryDate: string;
  manufacturer: { name: string; motherCompany: string | null } | null;
...
export default function InventoryPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const response = await fetch(`/api/products/${id}`, { method: 'DELETE', headers: { 'Cache-Control': 'no-cache' } });
      const data = await response.json();
  ...
  const handlePrintPDF = () => {
    const printContent = `
      <html>
        <head>
          <title>Glorious Pharma - Inventory Report</title>
          <style>
        ...
        <body>
          <div class="header">
            <div>
              <div class="company">GLORIOUS PHARMA.CO.LTD</div>
              <div class="meta">Plot No. 7316, Light Industrial Area, Chibengele Road, Lusaka</div>
            </div>
            <div class="meta" style="text-align:right">
              <div><strong>Inventory Report</strong></div>
              <div>Generated: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}</div>
              <div>Total: ${pagination.total} products | Showing: ${products.length}</div>
            </div>
          </div>
          <table>
            <thead>
            ...
            <tbody>
              ${products.map((p, i) => `
                <tr>
                  ...
                  <td class="${p.isExpired ? 'expired' : p.isExpiringSoon ? 'expiring' : ''}">${new Date(p.expiryDate).toLocaleDateString()}</td>
  ...
  const getTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      TABLET: 'bg-blue-100 text-blue-800', CAPSULE: 'bg-purple-100 text-purple-800',
      SYRUP: 'bg-green-100 text-green-800', INJECTION: 'bg-red-100 text-red-800',
      OINTMENT: 'bg-amber-100 text-amber-800', OTHER: 'bg-gray-100 text-gray-800'
    };
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <input
                ...
                className="w-full pl-10 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 transition-colors text-gray-900"
              />
              <svg className="w-5 h-5 text-gray-400 absolute left-3 top-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>
            <button type="submit" className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-medium hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg">
              Search
            </button>
            <button type="button" onClick={() => { setSearch(''); fetchProducts(true); }} className="px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 transition-all">
              Clear
            </button>
          </form>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-gray-600 font-medium whitespace-nowrap">Show per page:</label>
              <select
                value={limit}
                onChange={(e) => handleLimitChange(Number(e.target.value))}
                className="px-3 py-2 border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-900 focus:outline-none focus:border-blue-500 transition-colors"
              >
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <button onClick={() => fetchProducts(true)} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-medium">Retry</button>
            </div>
          ) : products.length === 0 ? (
            <div className="p-12 text-center">
              <div className="text-6xl mb-4">💊</div>
              <p className="text-gray-500 text-lg mb-4">No products found.</p>
              {search ? <p className="text-gray-400">Try adjusting your search terms</p> : (
                <Link href="/inventory/add" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-medium">
                    ...
                    {products.map((product, index) => (
                      <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          {product.currentStock <= product.minStock && <div className="text-xs text-red-600 mt-1">Below minimum</div>}
                        </td>
                        <td className="px-6 py-4 font-semibold text-gray-900">K{product.price.toFixed(2)}</td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center ${product.isExpired ? 'text-red-600' : product.isExpiringSoon ? 'text-yellow-600' : 'text-green-600'}`}>
                            <span className={`w-2 h-2 rounded-full mr-1.5 ${product.isExpired ? 'bg-red-600' : product.isExpiringSoon ? 'bg-yellow-600' : 'bg-green-600'}`}></span>
                            {new Date(product.expiryDate).toLocaleDateString()}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-3">
                            <Link href={`/inventory/${product.id}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm flex items-center">
                            ...
                            <Link href={`/inventory/${product.id}/edit`} className="text-green-600 hover:text-green-800 font-medium text-sm flex items-center">
                              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                              </svg>
                              Edit
                            </Link>
                            <button onClick={() => handleDelete(product.id, product.name)} className="text-red-600 hover:text-red-800 font-medium text-sm flex items-center">
                              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between flex-wrap gap-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      ← Prev
                    </button>
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                            pagination.page === pageNum ? 'bg-blue-600 text-white' : 'border-2 border-gray-200 text-gray-700 hover:bg-gray-50'
                          }`}
                        >
                          {pageNum}
                        </button>
