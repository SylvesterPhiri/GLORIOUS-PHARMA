...
// ─── AuditLogger (inline — no external import needed) ────────────────────────

async function logAudit(data: {
  action: string; entityType: string; entityId?: string;
  userId?: string; description?: string; newData?: any;
}) {
  try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // SQLite: no mode:'insensitive' — use plain contains (SQLite is case-insensitive for ASCII by default)
    if (search) {
      where.OR = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isHistorical && affectsInventory) {
      for (const item of data.items as InvoiceItemInput[]) {
        const product = await prisma.product.findUnique({ where: { id: item.productId! }, select: { currentStock: true, name: true } });
        if (!product) return NextResponse.json({ error: `Product not found: ${item.productId}` }, { status: 404 });
        if (product.currentStock < item.quantity) {
          return NextResponse.json({ error: `Insufficient stock for ${product.name}. Available: ${product.currentStock}, Required: ${item.quantity}` }, { status: 400 });
        }
      }
    }
    ...
    for (const item of data.items as InvoiceItemInput[]) {
      ...
      const totalPrice  = (quantity - freeSamples) * unitPrice;
      subTotal += totalPrice;
      ...
      for (const itemData of invoiceItemsData) {
        await tx.invoiceItem.create({
          data: {
            ...
            productSnapshot: itemData.productSnapshot, quantity: itemData.quantity,
            freeSamples: itemData.freeSamples, unitPrice: itemData.unitPrice, totalPrice: itemData.totalPrice,
          },
        });
        if (!isHistorical && affectsInventory && itemData.productId) {
          await tx.product.update({ where: { id: itemData.productId }, data: { currentStock: { decrement: itemData.quantity } } });
        }
      }
    ...
    await logAudit({
      ...
      newData:     { invoiceNumber, total, status: data.status, clientId: data.clientId },
    });
